...
<style>
    .inverst-clock{
        background:url("../../../images/account/clock.png") no-repeat;
	...
	<article id="List" class="inverst" v-if="map">
        <header>
            <ul class="center">
                <li>
                    <p>当前投资(元)</p>
                    <em>{{map.drMemberFunds.investAmount|localeString}}</em>
                </li>
                <li>
                    <p>待收收益(元)</p>
                    <em>{{map.drMemberFunds.winterest}}</em>
                </li>
            </ul>
        </header>
        <nav>
		...
		mounted(){
		    ...
		    this.setTitle({
                ...
                button : {
                    ...
                    class:"inverst-clock",
                    event(){
                        self.go().myAccount.myInverst.history();
                    }
                }
            });
        },
