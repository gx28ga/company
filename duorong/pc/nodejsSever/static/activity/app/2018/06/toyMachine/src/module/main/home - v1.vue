...
<script>
    ...
    /*const createIndice = odds => {
        ...
        //最小公倍数
        const minCm = (a, b) => {
            return a * b / minCd(a, b);
        };
        let intOdds = odds.map(item => {return Math.floor(item * 100);});
        let arrMinCd = intOdds.slice(1).reduce((curMinCd, item) => {
            return minCd(curMinCd, item);
        }, intOdds[0]);
        intOdds = intOdds.map(item => {return item / arrMinCd;});
        //生成随机大数组
        //随机取出目标公仔的index
        //生成其他公仔的随机序列

        const len = cToyClass.length;
        let arr = [], org = [];
        for(let i = 0; i < len; i++) {
            org.push(i);
        }
        for(let i = 0; i < len; i++) {
            const curLen = org.length;
            const idx = Math.floor(Math.random() * curLen);
            arr.push(org[idx]);
            org = org.filter((item, index) => {return index != idx});
        }
        return arr.slice(0);
    };*/
    const createIndice = () => {
        ...
        methods: {
            ...
            getMyChance: function () {
                ...
                requestService(httpChance, params, _data => {
                    ...
                    /*if (lottery.length === cToyClass.length) {
...
//                this.toyIndice = createIndice(this.toyOdds);
                this.toyIndice = createIndice();
            ...
            onPlayOnce: function () {
                ...
                infiniteMove(conbox.id,'btnl','btnr','BoxSwitch',true,'left',true,cMoveOffset,cMoveTime,cMoveItv,cShowLen, () => {this.moveCount++;});
