/* 应收账款收益权转让及回购协议 */
mainModule.controller('baoliProtocolCtrl', ['$rootScope','$scope','$filter','resourceService','$localStorage','$location', function($rootScope,$scope,$filter,resourceService,$localStorage,$location) {
      ...
      case "baoli":
        ...
        if(IS_MOBILE){
          window.localStorage.setItem("mapinfo",JSON.stringify($scope.info));
            window.location.href = '/main/html/protocol/mobile/baoli.html';
            console.log(mapinfo)
        }
        break;
    };
  ...
  if ($scope.ids.pid != undefined && $scope.ids.uid != undefined && $scope.ids.investId != undefined) {
    resourceService.queryPost($scope,$filter('交互接口对照表')('baoli'),$scope.ids,'baoli');
  }
}]);
