/*lee 产品详情页*/
mainModule.controller('billDetail2Ctrl', ['$rootScope','$scope', '$state', '$localStorage', 'resourceService','$filter','communicationService','$timeout','$location','ngDialog','$element','MAIN_MENU',function($rootScope,$scope, $state, $localStorage,resourceService,$filter,communicationService,$timeout,$location,ngDialog,$element,MAINMENU) {
	...
	$scope.$on('resourceService.QUERY_POST_MYEVENT', function(event,data,type) {
		switch(type){
			...
			case "票据详情":
				...
				$scope.expireDate=data.map.info.expireDate-86400000;
				...
				$scope.XQ = data.map;
				$scope.pics = data.map.picList;//图片
				$scope.file=data.map.textList;//文件
				$scope.table=data.map.extendTableList;//表格
				$scope.doc=data.map.extendTextList;//文档
				// 段落格式化
				// var content=$filter('截取段落')($scope.doc.content);
				// $scope.doc.content=content;
                ...
                //////////////////////////////PC前端优化需求 - 产品详情页替换‘^’为换行 Start /////////////////////////////////////
				setTimeout(function () {
					var content = document.getElementsByClassName("fileSpeci")[0].innerHTML;
					content = content.replace(/\^\r\n/g, "<br>");
					content = content.replace(/\^\r/g, "<br>");
					content = content.replace(/\^\n/g, "<br>");
					content = content.replace(/\^/g, "<br>");
                    document.getElementsByClassName("fileSpeci")[0].innerHTML = content;
                }, 200);
                //////////////////////////////PC前端优化需求 - 产品详情页替换‘^’为换行 End   /////////////////////////////////////
			break;
			case "关闭立即投资":
			...
			case "确认投资":
				...
				// $scope.task= (typeof data.map == 'Object' && data.map.hasOwnProperty('cmallResult')) ? data.map.cmallResult : {};//任务
...
function renderFinish(){
	...
	$scope.onClick=function (type,event,num) {
		switch(type){
			...
			break;
			case "登录":
				$filter("跳转页面")('denLu','main.billDetail2','dl');
			break;
			case "确认投资":
				if(isSubmin){
					isSubmin=false;
	...
	if ($location.$$search.id == 2) {
		...
		if($location.$$search.id != undefined){
			$localStorage.pathUrl = 'main.bankBillList2';
