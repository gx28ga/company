...
/*自动投标设置-账户保留余额*/
        .filter('autoTbServerError', function() {
        ...
        .filter('denLuUserNameError', function() {
            return function(code,name) {
                var error = {
                    1001 : "账号或密码为空",
                    1003 : "账号或密码错误",
	                9918 : "您的身份信息不符合本平台注册条件，请确认",
                    1004 : "密码错误次数超限,请30分钟后重试"
                };
                code.$valid = false;
                var text='';
                if(code.$error["required"]){
        ...
        .filter('确认投资服务器Error', function() {
            return function(code) {
                var error = {
                        ...
                        9919 : {text:"未满18岁用户无法进行投资",classCode:'error'},
                        6666 : {classCode: 'error'},
                        9999 : "系统错误",
                        test:"网络错误"
                };
                return error[code];
