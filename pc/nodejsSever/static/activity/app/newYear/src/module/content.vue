...
import HTTP from '../js/URL';
	...
	export default {
		...
		methods : {
			...
			getStatus : function(callback , obj){
				if(this.status == 2 || (obj && (obj.name == 'exchange'||obj.name=='mingxi')) ){ ///进行中
					callback();
				}else{//
					var txt;
			...
			,vipPop : function( callback ){// vip 无资格
				this.getVip(function(vipStatus){
					if(vipStatus==true){
						...
						var pop = new Pop({
							title : '提示',
							content : "<p>亲，由于您是渠道VIP用户<br>所以不能参加此次活动<br>敬请谅解！</p>",
							width: '70%',
							okBtn : true,
							cancelBtn:false,
							okTxt:'确定',
			...
			,investment : function(callback){//投资提示
				...
				if(window.localStorage){
					var inves = window.localStorage.getItem('investment');
					if(!inves){
