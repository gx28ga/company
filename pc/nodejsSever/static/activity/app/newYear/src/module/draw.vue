...
<script>
	...
	export default {
		...
		ready(){
			var self = this , 
				money;
			function winning(d,tenList){
				if( (d.btnStatus == 'ten' && tenList && tenList.length>0) || (d.btnStatus != 'ten' && void 0 != tenList) ){
					self.$dispatch('pop',{
						name:'winning',
						obj : d,
								...
								tenList.map((val , i)=>{
									setTimeout(function(){
										...
										setTimeout(function(){
											...
											content.find('.num').html('10次共获得：'+num+'金币')
										},50)
									},i*300);
					...
					self.$dispatch('pop',{name:'winningError'});
				}
			}
			this.$once('elementDone', function(T,Pop){
				...
				var d = new draw({
					...
					timeoutCallback : function(){ //超时或者其他异常，比如程序出错，接口请求不到数据等。
						winning(d,tenList);
					}
				});
