...
<script>
    ...
    const cMoveTime = 220;
    const cAnmSpeed = 320;
    const cWaitMoveCount = 6;
    const cWaitMoreTime = 160;
    const cLiftTime = 800;
    const cMoveItv = 0;
    const cShowLen = 3;
    ...
    import hash from'../../common/md5.min';
...
//                document.querySelectorAll(".conbox:not(#BoxUl)").forEach(node => {container.removeChild(node);});
                [].forEach.call(document.querySelectorAll(".conbox:not(#BoxUl)"), node => {container.removeChild(node);});
                let clone = conbox.cloneNode(true);
                clone.style.display = "block";
                this.toyIndice = createRandomOrder(this.gameMode, this.toyOdds, cSnachPos - 1);
            ...
            onPlayOnce: function () {
                ...
                infiniteMove(conbox.id,'btnl','btnr','BoxSwitch',true,'left',true,cMoveOffset,cMoveTime,cMoveItv,cShowLen, coordinator.checkStop, () => {
                    this.moveCount++;
                    if (this.moveCount === cWaitMoveCount) {
                        setTimeout(() => {
                            jQuery(".boom").animate({top: '0px'}, cAnmSpeed, (() => {
                                let runTimes = 0;
                                return () => {
                                    if (runTimes++ > 0) return;
                                    jQuery(`#${conbox.id}`).find("ul").stop(true);
                                    coordinator.setStop();
                                }
                            })());
                        }, cWaitMoreTime);
...
//                        cartoon1: this.targetIndice[0] + 1
                        key1: hash(this.chanceNum + '' + (this.targetIndice[0] + 1))
                    };
                    let params = this.gameMode > 0 ?
//                        {...baseParam, ...{cartoon2: this.targetIndice[1] + 1}} :
                        {...baseParam, ...{key2: hash(this.chanceNum + '' + (this.targetIndice[1] + 1))}} :
                        baseParam;
                            ...
                            if (timerOn == false) {
                                ...
                                if (success == false) {
                                    ...
                                    if (stCode) {
                                        ...
                                        bus.$once("closePopEvent", this.__prepareNewGame);
                                    }
                                    return false;
                                }
                                const {totalchance, prizeType, uuid} = _data.map;
