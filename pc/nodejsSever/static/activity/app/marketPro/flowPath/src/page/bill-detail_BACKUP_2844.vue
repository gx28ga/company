...
=======
	  			...
	  			this.pinfo = data.map.rookieList[0];
	  			self.profit = T.isNumber2((this.pinfo.rate +this.pinfo.activityRate)/100/365*100*this.pinfo.deadline)
		  		T.ajax({
		  			url:'/detail',
		  			data : {time : new Date(),id:this.pinfo.id},
		  			callback : true,
		  		}).then(data=>{
		  			this.info.maxPurchaseAmount = data.map.info.maxAmount;
		  		})
	  		})
	  		T.ajax({
	  			url:'/v1/current/product/info.do',
>>>>>>> 4ab6e49c103d1ca6aebe5555547e9f9ac2a9cf63
	  			data : {time : new Date()},
	  			callback : true,
	  		}).then(data=>{
			  				...
			  				if(self.amount<=self.info.balance){
			  					if(self.amount<=self.info.maxPurchaseAmount){
					                if(self.reg.test(self.amount)==true){
					  					...
					  					T.localStorage.setItem('prate',self.pinfo.rate+self.pinfo.activityRate);
					  					T.localStorage.setItem('profit',self.profit);
					  					T.localStorage.setItem('pdeadline',self.pinfo.deadline);
					  					T.localStorage.setItem('pname',self.pinfo.fullName);
					                	self.amounterror = ''; 
							  			requirePop('inputsd',{
							  				props: {
						                        title : '请输入交易密码',
...
=======
                    <span class="img-size" data-fontSize='120'>{{pinfo.rate}}</span>
                    %
                    <i class="img-size" data-fontSize='72'>+</i>
                    <span class="img-size" data-fontSize='120'>{{pinfo.activityRate}}</span>
>>>>>>> 4ab6e49c103d1ca6aebe5555547e9f9ac2a9cf63
                    % <br>
...
=======
                    <span class="img-size" data-fontSize='80'>{{pinfo.deadline}}</span>
>>>>>>> 4ab6e49c103d1ca6aebe5555547e9f9ac2a9cf63
                    天<br>
...
=======
	    	<div class="img-size" data-lineHeight='78'>起投金额<span>{{pinfo.leastaAmount | currency}}元，且以{{pinfo.increasAmount | currency}}的倍数递增</span></div>
            <div class="img-size" data-lineHeight='78'>剩余可投<span>{{pinfo.surplusAmount | currency}}</span></div>
	    	<div class="img-size" data-lineHeight='78'>产品总额<span>{{pinfo.amount | currency}}</span></div>
>>>>>>> 4ab6e49c103d1ca6aebe5555547e9f9ac2a9cf63
